...
def lookup_one_hop(graph, fromAddr) {
    g = graph.traversal()

    for (Vertex v : g.V().has('address', 'address_hash', fromAddr).out().next()) {
        if (g.V(v.id()).inE().count().next() > 20 || g.V(v.id()).outE().count().next() > 20) { continue }
        for (Vertex twoHopV : g.V(v.id()).out().next()) {
            if (g.V(twoHopV.id()).inE().count().next() > 20 || g.V(twoHopV.id()).outE().count().next() > 20) {
                continue
            }
            println("from to " + fromAddr + " -> " + g.V(v.id()).property("address_hash") +
                    " -> " + g.V(twoHopV.id()).property("address_hash"))
        }
    }
}
