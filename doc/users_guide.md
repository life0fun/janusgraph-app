...
- Usage:
  ...
  - `<janusgraph-config-file>`: Properties file with the JanuGraph configuration, see https://docs.janusgraph.org/configs/. At minimal, set storage.backend, storage.hostname, and storage.batch-loading=true. The config-file is passed to JanusGraph graph = JanusGraphFactory.open(conf);  
